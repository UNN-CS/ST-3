...
TimedDoor::TimedDoor(int time) : m_timeout(time) { m_adapter.SetTimedDoorData(time, weak_from_this()); }
...
void DoorTimerAdapter::SetTimedDoorData(int baseSleepTime, std::weak_ptr<TimedDoor> door) {
  m_door = std::move(door);
  m_baseSleepTime = baseSleepTime;
}

void DoorTimerAdapter::Timeout() { m_timer.tregister(m_baseSleepTime, weak_from_this()); }
...
void Timer::sleep(int time) { std::this_thread::sleep_for(std::chrono::seconds(time)); }
