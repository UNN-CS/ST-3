...
TEST(AdapterTest, DoorClosedNoException)
{
    ...
    door.lock();
    EXPECT_NO_THROW(std::this_thread::sleep_for(std::chrono::milliseconds(100)));
}
...
TEST(AdapterTest, DoorOpenThrowsException)
{
    ...
    EXPECT_THROW({
        door.unlock();
        std::this_thread::sleep_for(std::chrono::milliseconds(200)); }, std::runtime_error);
}
...
TEST(AdapterTest, MultipleOpenClose)
{
    ...
    door.lock();
    door.unlock();
    EXPECT_THROW(std::this_thread::sleep_for(std::chrono::milliseconds(200)), std::runtime_error);
}
