...
class vasilev_s_timed_door_test : public ::testing::Test {
 ...
 public:
    vasilev_s_timed_door_test() : door(5), timer() {}
};

TEST_F(vasilev_s_timed_door_test, Test_DoorTimerAdapter_Construction) {
    EXPECT_NO_THROW(DoorTimerAdapter adapter(door));
}

TEST_F(vasilev_s_timed_door_test, Test_Timer_RegisterWithNullClient) {
    EXPECT_THROW(timer.tregister(5, nullptr), std::invalid_argument);
}

TEST_F(vasilev_s_timed_door_test, Test_Door_AfterCreation_IsClosed) {
...
TEST_F(vasilev_s_timed_door_test, Test_Door_LockAfterTimerRegister) {
    timer.tregister(door.getTimeOut(), timerClient);
    EXPECT_ANY_THROW(door.lock());
}

TEST_F(vasilev_s_timed_door_test, Test_Door_LockAfterUnlock_DoesNotThrow) {
    door.unlock();
...
TEST_F(vasilev_s_timed_door_test, Test_Door_LockBeforeTimeout) {
    door.unlock();
    timer.tregister(door.getTimeOut(), timerClient);
...
TEST_F(vasilev_s_timed_door_test, Test_Door_ThrowStateAfterTimeout) {
    door.unlock();
    timer.tregister(door.getTimeOut(), timerClient);
...
TEST_F(vasilev_s_timed_door_test, Test_Door_LockBeforeTimeout_T) {
    door.unlock();
    timer.tregister(door.getTimeOut(), timerClient);
    std::this_thread::sleep_for(std::chrono::seconds(3));
    EXPECT_NO_THROW(door.lock());
